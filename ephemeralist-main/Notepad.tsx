...
const Notepad: React.FC<NotepadProps> = ({ tasks, setTasks, onFocusTask, activeTaskId, onAICommand, artifacts, onOpenArtifact, onOpenSpace }) => {
    ...
    setTasks(prev => prev.map(t => {
      if (t.id === id) {
        ...
        if (updates.content !== undefined) {
          const { tag, deadline, deadlineISO } = parseText(updates.content);
          updated.tag = tag;
          if (deadline) updated.meta = { ...updated.meta, deadline, deadlineISO };
        }
        return updated;
      }
      return t;
  ...
  const handleKeyDown = (e: React.KeyboardEvent, indexInVisible: number, task: Task) => {
    ...
    if (e.key === 'Enter') {
      ...
      // We need the index in the full tasks array to insert correctly
